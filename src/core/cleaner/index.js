...
// Pub/Sub (sessions)
const subS = new Redis(config.redis.sessions);
const pubS = new Redis(config.redis.sessions);

// Pub/Sub (users)
const subU = new Redis(config.redis.users);
const pubU = new Redis(config.redis.users);
...
// Users
subU.rawCall([
  'subscribe', `__keyevent@${config.redis.users.db}__:del`
], async function(e, data) {
  let id = extract(data);
  if (id) {
    pubS.rawCall([
